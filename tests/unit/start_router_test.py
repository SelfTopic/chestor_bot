from src.bot.routers.start_router import start_handler 
...
async def test_start_router():
    ...
    mock_user_service.get.return_value = MagicMock(id=1, first_name="Alex", telegram_id=123)
    ...
    mock_dialog_service.text.assert_called_once_with(
        key="start", 
        name="John"
    )