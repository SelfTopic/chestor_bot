...
class CooldownService(Base):
    async def get_user_cooldown(
        ...
        logger.debug(
            f"Called method get_user_cooldown. Params: telegram_id={telegram_id}, cooldown_name={cooldown_name}"
        )

        user_cooldown = await self.cooldown_repository.get_user_cooldown(
    ...
    async def get_active_cooldown(
        ...
        logger.debug(
            f"Called method get_active_cooldown. Params: telegram_id={telegram_id}, cooldown_name={cooldown_name}"
        )

        logger.debug("User cooldown_repository. Calling method get_active_cooldown.")

        user_cooldown = await self.cooldown_repository.get_active_cooldown(
            telegram_id=telegram_id, cooldown_name=cooldown_name
        )
    ...
    async def get_cooldown(
        self, cooldown_name: str | None = None, cooldown_id: int | None = None
    ):
        logger.debug(
            f"Called method get_cooldown. Params: cooldown_name={cooldown_name}, cooldown_id={cooldown_id}"
        )
        ...
        if not parameter:
            logger.error(
                "Arguments is invalid - both cooldown_name and cooldown_id are None"
            )
            raise ValueError("Arguments is invalid")

        logger.debug(f"Searching cooldown with parameter: {parameter}")

        cooldown = await self.cooldown_repository.get_cooldown(
    ...
    async def set_cooldown(self, telegram_id: int, cooldown_type: str):
        logger.debug(
            f"Called method set_cooldown. Params: telegram_id={telegram_id}, cooldown_type={cooldown_type}"
        )
    ...
    async def reset_cooldown(self, telegram_id: int, cooldown_type: str):
        logger.debug(
            f"Called method reset_cooldown. Params: telegram_id={telegram_id}, cooldown_type={cooldown_type}"
        )
        ...
        new_cooldown = await self.cooldown_repository.reset_cooldown(
            user_id=user.id, cooldown_type=cooldown_type
        )

        logger.debug(
            f"Cooldown reset successfully: {new_cooldown is not None}. End process."
        )
        return new_cooldown
    ...
    async def is_end_cooldown(self, telegram_id: int, cooldown_type: str):
        ...
        is_cooldown = await self.cooldown_repository.is_cooldown_active(
            user_id=telegram_id, cooldown_type=cooldown_type
        )
