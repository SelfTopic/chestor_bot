...
class LevelUpService:
    ...
    async def add_progress(self, telegram_id: int, delta: float) -> ProgressResult:
        ...
        РОВНО один раз, независимо от размера delta (уровень никогда не
        перепрыгивает больше чем на 1 за одно событие - излишек выше 100%
        отбрасывается, см. apply_level_progress). Это единственный (пока не
        введён реальный источник - бой/поедание гулей) способ реально
        ПРОДВИНУТЬ прогресс, в отличие от /force_levelup, который левелапит
        напрямую в обход прогресса."""
