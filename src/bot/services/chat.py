...
class ChatService(Base):
    ...
    async def get_by_telegram_id(self, telegram_id: int) -> Optional[Chat]:
        ...
        chat = await self.chat_repository.\
            get_chat_by_telegram_id(telegram_id=telegram_id)
    ...
    async def set_chat_rules(
        ...
        if len(rules) < 1 or len(rules) > 4000:
            raise ChatRulesError("Кол-во символов в правилах не может быть меньше 1 и больше 4000 символов.")
        
        chat = await self.chat_repository\
            .set_chat_rules(telegram_id=telegram_id, rules=rules)
    ...
    async def set_chat_welcome_message(
        ...
        if len(welcome_message) < 0 or len(welcome_message) > 4000:
            raise ChatMemberUpdateMessageError("Кол-во символов в приветственном сообщении не может быть меньше 1 и больше 4000 символов.")
        
        chat = await self.chat_repository\
            .set_chat_welcome_message(telegram_id=telegram_id, welcome_message=welcome_message)
    ...
    async def delete_chat_welcome_message(self, telegram_id: int) -> Chat:
        ...
        if not before_chat.welcome_message:
            raise ChatMemberUpdateMessageError("В чате уже отсутствует приветственное сообщение.")
        
        chat = await self.chat_repository.delete_chat_welcome_message(telegram_id=telegram_id)
    ...
    async def set_chat_goodbye_message(
        ...
        if len(goodbye_message) < 0 or len(goodbye_message) > 4000:
            raise ChatMemberUpdateMessageError("Кол-во символов в прощальном сообщении не может быть меньше 1 и больше 4000 символов.")
        
        chat = await self.chat_repository\
            .set_chat_goodbye_message(telegram_id=telegram_id, goodbye_message=goodbye_message)
    ...
    async def delete_chat_goodbye_message(self, telegram_id: int) -> Chat:
        ...
        if not before_chat.welcome_message:
            raise ChatMemberUpdateMessageError("В чате уже отсутствует прощальное сообщение.")
        
        chat = await self.chat_repository.delete_chat_goodbye_message(telegram_id=telegram_id)
